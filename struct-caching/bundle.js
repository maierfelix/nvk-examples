...
var nvk = _interopDefault(require('nvk'));
var essentials = _interopDefault(require('nvk-essentials'));
...
let posVertexBindingDescr = VkVertexInputBindingDescription("0x0");
...
let posVertexAttrDescr = VkVertexInputAttributeDescription("0xC");
...
function createShaderModule(shaderSrc, shaderModule) {
  let shaderModuleInfo = VkShaderModuleCreateInfo("0x1C");
...
function getMemoryTypeIndex(typeFilter, propertyFlag) {
  let memoryProperties = VkPhysicalDeviceMemoryProperties("0x44");
...
function createVertexBuffer(buffer, bufferMemory, byteLength) {
  let bufferInfo = VkBufferCreateInfo("0x24C");
  ...
  let memoryRequirements = VkMemoryRequirements("0x284");
  ...
  let memAllocInfo = VkMemoryAllocateInfo("0x29C");
...
// app info
let appInfo = VkApplicationInfo("0x2BC");
...
// create info
let createInfo = VkInstanceCreateInfo("0x2EC");
...
let deviceFeatures = VkPhysicalDeviceFeatures("0x32C");
vkGetPhysicalDeviceFeatures(physicalDevice, deviceFeatures);

let deviceProperties = VkPhysicalDeviceProperties("0x408");
vkGetPhysicalDeviceProperties(physicalDevice, deviceProperties);

console.log(`Using device: ${deviceProperties.deviceName}`);

let deviceMemoryProperties = VkPhysicalDeviceMemoryProperties("0x740");
vkGetPhysicalDeviceMemoryProperties(physicalDevice, deviceMemoryProperties);

let queueFamilyCount = { $: 0 };
vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, queueFamilyCount, null);

let queueFamilies = [...Array(queueFamilyCount.$)].map(() => VkQueueFamilyProperties("0x948"));
vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, queueFamilyCount, queueFamilies);

let surfaceCapabilities = VkSurfaceCapabilitiesKHR("0x960");
...
let deviceInfo = VkDeviceCreateInfo("0x994");
deviceInfo.sType = VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO;
deviceInfo.queueCreateInfoCount = 1;
deviceInfo.pQueueCreateInfos = [deviceQueueInfo];
deviceInfo.enabledExtensionCount = deviceExtensions.length;
deviceInfo.ppEnabledExtensionNames = deviceExtensions;
deviceInfo.pEnabledFeatures = VkPhysicalDeviceFeatures("0x9DC");
...
let swapchainInfo = VkSwapchainCreateInfoKHR("0xAB8");
...
for (let ii = 0; ii < amountOfImagesInSwapchain.$; ++ii) {
  let imageViewInfo = VkImageViewCreateInfo("0xB20");
...
let shaderStageInfoVert = VkPipelineShaderStageCreateInfo("0xB70");
...
let shaderStageInfoFrag = VkPipelineShaderStageCreateInfo("0xBA0");
...
let vertexInputInfo = VkPipelineVertexInputStateCreateInfo("0xBD0");
...
let inputAssemblyStateInfo = VkPipelineInputAssemblyStateCreateInfo("0xC00");
inputAssemblyStateInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;
inputAssemblyStateInfo.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
inputAssemblyStateInfo.primitiveRestartEnable = false;

let viewport = VkViewport("0xC20");
viewport.x = 0;
viewport.y = 0;
...
let scissor = VkRect2D("0xC38");
scissor.offset.x = 0;
scissor.offset.y = 0;
scissor.extent.width = win.width;
scissor.extent.height = win.height;

let viewportStateInfo = VkPipelineViewportStateCreateInfo("0xC48");
...
let rasterizationInfo = VkPipelineRasterizationStateCreateInfo("0xC78");
...
let multisampleInfo = VkPipelineMultisampleStateCreateInfo("0xCB8");
...
let colorBlendAttachment = VkPipelineColorBlendAttachmentState("0xCE8");
...
let colorBlendInfo = VkPipelineColorBlendStateCreateInfo("0xD08");
...
let pipelineLayoutInfo = VkPipelineLayoutCreateInfo("0xD40");
...
let attachmentDescription = VkAttachmentDescription("0xD70");
...
let attachmentReference = VkAttachmentReference("0xD94");
attachmentReference.attachment = 0;
attachmentReference.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

let subpassDescription = VkSubpassDescription("0xD9C");
...
let subpassDependency = VkSubpassDependency("0xDE4");
...
let renderPassInfo = VkRenderPassCreateInfo("0xE00");
...
let graphicsPipelineInfo = VkGraphicsPipelineCreateInfo("0xE40");
...
for (let ii = 0; ii < amountOfImagesInSwapchain.$; ++ii) {
  let framebufferInfo = VkFramebufferCreateInfo("0xED0");
...
let cmdPoolInfo = VkCommandPoolCreateInfo("0xF10");
cmdPoolInfo.flags = 0;
...
let cmdBufferAllocInfo = VkCommandBufferAllocateInfo("0xF28");
...
let cmdBufferBeginInfo = VkCommandBufferBeginInfo("0xF48");
...
for (let ii = 0; ii < cmdBuffers.length; ++ii) {
  ...
  let clearValue = VkClearValue("0xF68");

  let renderPassBeginInfo = VkRenderPassBeginInfo("0xF78");
...
let semaphoreInfo = VkSemaphoreCreateInfo("0xFB8");
...
function drawFrame() {
  ...
  let submitInfo = VkSubmitInfo("0xFD0");
  ...
  let presentInfo = VkPresentInfoKHR("0x1018");
