...
let posVertexBindingDescr = VkVertexInputBindingDescription();
...
let posVertexAttrDescr = VkVertexInputAttributeDescription();
...
function createShaderModule(shaderSrc, shaderModule) {
  let shaderModuleInfo = VkShaderModuleCreateInfo();
...
function createVertexBuffer(buffer, bufferMemory, byteLength) {
  let bufferInfo = VkBufferCreateInfo();
  ...
  let memAllocInfo = VkMemoryAllocateInfo();
...
let deviceMemoryProperties = VkPhysicalDeviceMemoryProperties();
...
let shaderStageInfoVert = VkPipelineShaderStageCreateInfo();
...
let shaderStageInfoFrag = VkPipelineShaderStageCreateInfo();
...
let inputAssemblyStateInfo = VkPipelineInputAssemblyStateCreateInfo();
...
let viewportStateInfo = VkPipelineViewportStateCreateInfo();
...
let rasterizationInfo = VkPipelineRasterizationStateCreateInfo();
...
let multisampleInfo = VkPipelineMultisampleStateCreateInfo();
...
let colorBlendInfo = VkPipelineColorBlendStateCreateInfo();
...
let attachmentDescription = VkAttachmentDescription();
...
let attachmentReference = VkAttachmentReference();
...
let subpassDependency = VkSubpassDependency();
...
let graphicsPipelineInfo = VkGraphicsPipelineCreateInfo();
...
let cmdPoolInfo = VkCommandPoolCreateInfo();
cmdPoolInfo.flags = 0;
...
let cmdBufferAllocInfo = VkCommandBufferAllocateInfo();
...
let cmdBufferBeginInfo = VkCommandBufferBeginInfo();
...
for (let ii = 0; ii < cmdBuffers.length; ++ii) {
  ...
  let renderPassBeginInfo = VkRenderPassBeginInfo();
...
let semaphoreInfo = VkSemaphoreCreateInfo();
